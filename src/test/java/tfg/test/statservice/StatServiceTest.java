...
public class StatServiceTest {
	...
	public void findStatsTest(){
		...
		List<Stat> stats = statService.findByRoute(route.getRouteId(), 0, 2);
		...
		statService.saveStat(stat1);
		stats = statService.findByRoute(route.getRouteId(), 0, 2);
		assertTrue(stats.size()==2);
		assertEquals(stats.get(0),stat);
		assertEquals(stats.get(1),stat1);
		stats = statService.findByRoute(route.getRouteId(), 0, 1);
		assertTrue(stats.size()==1);
		assertEquals(stats.get(0),stat);
		stats = statService.findStatByRouteAndDate(route.getRouteId(), 0, 2, dateStart, dateEnd);
		assertTrue(stats.size()==2);
		assertEquals(stats.get(0),stat);
		assertEquals(stats.get(1),stat1);
		stats = statService.findStatByRouteAndDate(route.getRouteId(), 0, 1, dateStart, dateEnd);
		assertTrue(stats.size()==1);
		assertEquals(stats.get(0),stat);
	...
	@Test
	public void getOldMiddle(){
		List<Long> balances = new ArrayList<Long>();
		balances.add(3l);
		balances.add(5l);
		balances.add(7l);
		balances.add(9l);
		Double valueBD=statService.getOldMiddle(balances);
		Double value=6.0;
		assertEquals(valueBD,value);
	}
	
	@Test
	public void getTypicalDesviation(){
		List<Long> balances = new ArrayList<Long>();
		balances.add(3l);
		balances.add(5l);
		balances.add(7l);
		balances.add(9l);
		Double valueBD=statService.getTypicalDeviation(balances);
		Double value=2.581988897471611;
		assertEquals(valueBD,value);
	}
}
